...
- Scheduler runs approximately every minute while the application is open. It generates tickets due within two UTC calendar days, including overdue work, chooses the active technician with the fewest unresolved tickets and uses the equipment-assigned active approver, and prevents duplicate open cycles. Three-days-away work is excluded. Daily equipment can be eligible immediately.
